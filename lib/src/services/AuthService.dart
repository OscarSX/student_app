...
class AuthService with ChangeNotifier{
  ...
  Future<DocumentSnapshot> getUser() async {    
    ...
    if(user!=null){  
      var emailExistQuery = Firestore.instance.collection('alumnos')
        .where('correo', isEqualTo: user.email);
      var querySnapshots = await emailExistQuery.getDocuments();
      DocumentSnapshot userFromFirestore = await Firestore.instance.collection('alumnos').document(querySnapshots.documents[0].documentID).get();                                                
  ...
  Future<DocumentSnapshot> signInWithEmailAndPassword(String matricula, String password ) async {    
    ...
    var userExistQuery = _db.collection('alumnos')
  ...
  Future<FirebaseUser> register(matricula,email, password,usuario,codigo,area) async {    
   ...
   void updateUserData(String uid,String correo,String area,String usuario) async {
    ...
    Firestore.instance.runTransaction((transaction) async {
      await transaction.update(Firestore.instance.collection('alumnos').document(uid),{
        'uid': uid,        
        'usuario': usuario,                                    
        'correo': correo,                                    
        'area':area,
     ...
     void addUser(String Matricula,String Codigo){   
      Firestore.instance.runTransaction((transaction) async {
          await transaction.set(Firestore.instance.collection('alumnos').document(), {
            'matricula':Matricula,            
            'estado': false,
            'codigo': Codigo,          
          });
